...
char const *toString(ProdDeclKind);
...
public:      // funcs
  GrammarAST(ASTList <TopForm > *_forms) : forms(_forms), allowContinuedNonterminals( false) {
  ...
  public:  bool allowContinuedNonterminals ;
};
...
public:      // funcs
  ProdDecl(SourceLoc _loc, ProdDeclKind _kind, ASTList <RHSElt > *_rhs, LocString *_actionCode) : loc(_loc), kind(_kind), rhs(_rhs), actionCode(_actionCode) {
  }
  ~ProdDecl();
...
public:      // funcs
  ...
  enum Kind { RH_NAME, RH_STRING, RH_PREC, RH_FORBID, NUM_KINDS };
  virtual Kind kind() const = 0;
  ...
  DECL_AST_DOWNCASTS(RH_string, RH_STRING)
  DECL_AST_DOWNCASTS(RH_prec, RH_PREC)
  DECL_AST_DOWNCASTS(RH_forbid, RH_FORBID)
...
class RH_forbid : public RHSElt {
public:      // data
  LocString tokName;

public:      // funcs
  RH_forbid(LocString *_tokName) : RHSElt(), tokName(_tokName) {
  }
  virtual ~RH_forbid();

  virtual Kind kind() const { return RH_FORBID; }
  enum { TYPE_TAG = RH_FORBID };

  virtual void debugPrint(std::ostream &os, int indent, char const *subtreeName = "tree") const;

  virtual RH_forbid *clone() const;
