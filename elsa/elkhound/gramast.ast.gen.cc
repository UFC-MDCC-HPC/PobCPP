...
#include "gramast.ast.gen.h"      // this module
...
char const *toString(ProdDeclKind x)
{
...
void RH_string::debugPrint(std::ostream &os, int indent, char const *subtreeName) const
{
  PRINT_HEADER(subtreeName, RH_string);
...
RH_string *RH_string::clone() const
{
  RH_string *ret = new RH_string(
    tag.clone(),
    str.clone()
  );
  return ret;
...
DEFN_AST_DOWNCASTS(RHSElt, RH_forbid, RH_FORBID)

RH_forbid::~RH_forbid()
{
}

void RH_forbid::debugPrint(std::ostream &os, int indent, char const *subtreeName) const
{
  PRINT_HEADER(subtreeName, RH_forbid);
...
RH_forbid *RH_forbid::clone() const
{
  RH_forbid *ret = new RH_forbid(
    tokName.clone()
  );
  return ret;
}
