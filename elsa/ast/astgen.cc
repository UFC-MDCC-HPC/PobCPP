...
inline bool contains(const set<string> &set, const string &member) {
...
set<string> listClassesSet;
set<ListClass*> listClasses;
...
class XmlParserGen {
  set<string> attributeNames;     // names of attributes of AST nodes
...
void HGen::emitVisitorInterface()
{
  ...
  out << "\n  // List 'classes'\n";
  for (set<ListClass*>::iterator iter = listClasses.begin();
       iter != listClasses.end();
       iter++) {
    ListClass *cls = *iter;
...
void XmlParserGen::emitXmlParserImplementation()
{
  ...
  for (set<string>::iterator attrIter = attributeNames.begin();
       attrIter != attributeNames.end(); attrIter++) {
    string const &attr = *attrIter;
    tokensOut << "  " << attr << "\n";
  }
}
...
void recordListClass(ListKind lkind, rostring className, CtorArg const *arg) {
  ...
  if (!contains(listClassesSet, cls->classAndMemberName)) {
    listClassesSet.insert(cls->classAndMemberName);
    listClasses.insert(cls);
  } else {
    delete cls;
  }
}
