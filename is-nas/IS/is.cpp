...
void	IntegerSort::Sorter::create_seq( double seed, double a )
{
	double x;
	int    i, k;
...
void IntegerSort::Sorter::full_verify( void )
{
    MPI_Status  status;
    MPI_Request request;
...
/*  Send largest key value to next processor  */
    if( my_rank > 0 ) {
     /* MPI_Irecv( &k,
                   ...
                   MPI_COMM_WORLD,
                   &request );*/
  		request = comm.irecv(my_rank-1, 1000, k);
		}
...
void IntegerSort::Sorter::rank( int iteration )
{
...
/*    MPI_Allreduce( bucket_size, 
                   ...
                   MPI_SUM,
                   MPI_COMM_WORLD );*/
		comm.allreduce(bucket_size, NUM_BUCKETS+TEST_ARRAY_SIZE, bucket_size_totals, MPI_SUM);
...
/*    MPI_Reduce( &timecounter,
                ...
                MPI_MAX,
                0,
                MPI_COMM_WORLD ); FIXME */
		comm.reduce(timecounter, maxtime, MPI_MAX, 0);
...
/*          MPI_Reduce( &timecounter,
                        ...
                        0,
                        MPI_COMM_WORLD );*/
						comm.reduce(&timecounter, &tmin, 1, MPI_MIN, 0);
/*          MPI_Reduce( &timecounter,
                        &tsum,
                        1,
                        MPI_DOUBLE,
                        ...
                        0,
                        MPI_COMM_WORLD );*/
						comm.reduce(&timecounter, &tsum, 1, MPI_SUM, 0);
/*          MPI_Reduce( &timecounter,
                        &tmax,
                        1,
                        MPI_DOUBLE,
                        ...
                        0,
                        MPI_COMM_WORLD );*/
						comm.reduce(&timecounter, &tmax, 1, MPI_MAX, 0);
            if( my_rank == 0 )
...
/*    MPI_Reduce( &itemp,
                ...
                0,
                MPI_COMM_WORLD ); FIXME */
		comm.reduce(itemp, passed_verification, MPI_SUM, 0);
...
IntegerSort::Sorter::Sorter(Communicator comm, int rank, int size) : my_rank(rank), comm_size(size), comm(comm) { }
...
int main( int argc, char **argv) {
	std::cerr << "parallel IS" << std::endl;
	MPI_Init(&argc, &argv);
	Communicator world;
	unsigned int size = world.size();
	unsigned int rank = world.rank();
	IntegerSort::Sorter s(world, rank, size);
	int ret=  s.sort();
	MPI_Finalize();
	return ret;
